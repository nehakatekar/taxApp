...
- Plain CSS in `src/index.css` - no CSS-in-JS, no Tailwind, no component
  ...
  All animation (blobs, count-up, parallax, reveal, nav compacting, scroll
  progress) is hand-rolled CSS + vanilla JS/IntersectionObserver - no
  animation library either. The one exception is the partners marquee, which
  uses `react-fast-marquee` (added on request).
