...
import { IconDocument, IconReceipt, IconChartUp, IconShieldCheck } from './icons.jsx'
    ...
    <div className="flip-card-wrap" data-reveal data-reveal-delay={index % 2}>
      <div className={`flip-card ${flipped ? 'flip-card--flipped' : ''}`}>
        <div className="flip-card__inner">
          ...
          >
            <span className="flip-card__image" aria-hidden="true">
              <Icon width={40} height={40} />
            </span>
            {/* A heading element isn't valid inside <button>'s content model,
                so the visible title here is styled text; the "real" <h3> for
                this card lives on the back face below, which keeps it once
                in the heading outline without breaking HTML validity. */}
          ...
          >
            ...
            <a href="#contact" className="btn btn--small" tabIndex={flipped ? 0 : -1}>
              Ask about this
            </a>
            <button
              type="button"
            ...
            >
              <span className="sr-only">Back to </span>{service.title}
            </button>
          </div>
        </div>
      </div>
